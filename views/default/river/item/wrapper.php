<?php
	...
	*/
	...
	//Add river itemtime
	$time_posted = elgg_view_friendly_time($vars['item']->posted);
	$time_posted = <<<EOT
...
EOT;
	...
	$avatar_icon = elgg_get_plugin_setting("avatar_icon","riverdashboard");
	if(!$avatar_icon) {
		$avatar_icon = "icon";
	}
	if($avatar_icon == "icon"){
		...
		<div class="river_item">
			<span class="river_item_useravatar">
				<?php
				echo elgg_view_entity_icon($vars['item']->subject_guid, 'tiny');
				//echo elgg_view_entity_icon(get_entity($vars['item']->subject_guid), 'large');
			//echo elgg_view_entity_icon("profile/icon",array('entity' => get_entity($vars['item']->subject_guid), 'size' => 'tiny'));
				?>
			</span>
			<?php echo $river_content ?>
